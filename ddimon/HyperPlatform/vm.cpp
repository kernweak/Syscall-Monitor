...
_IRQL_requires_max_(PASSIVE_LEVEL) static bool VmpIsXsaveAvailable();

_IRQL_requires_max_(PASSIVE_LEVEL) static bool VmpIsXsavesAvailable();
...
_Use_decl_annotations_ NTSTATUS VmInitialization() {
  ...
  // DdimonInitialization(shared_data->shared_sh_data);
...
_Use_decl_annotations_ static bool VmpIsXsaveAvailable() {
	PAGED_CODE();

	// If CPUID.1:ECX.XSAVE[bit 26]=1, then XSAVE operation is supported.
	int cpu_info[4] = {};
	__cpuid(cpu_info, 1);
	const CpuFeaturesEcx cpu_features = { static_cast<ULONG_PTR>(cpu_info[2]) };
	if (!cpu_features.fields.xsave)
		return false;

	return true;
}

_Use_decl_annotations_ static bool VmpIsXsavesAvailable() {
	PAGED_CODE();

	if (!VmpIsXsaveAvailable())
		return false;

	// If CPUID.0Dh:ECX.1:EDX.XSAVES[bit 3]=1, then XSAVES operation is supported.
	int cpu_info[4] = {};
	__cpuidex(cpu_info, 0xD, 1);
	const CpuFeaturesEaxXsave cpu_features_xsave = { static_cast<ULONG_PTR>(cpu_info[2]) };
	if (!cpu_features_xsave.fields.xsaves)
		return false;
...
_Use_decl_annotations_ static void *VmpBuildMsrBitmap() {
  ...
  const auto bitmap_write_low = bitmap_read_high + 1024;
  const auto bitmap_write_high = bitmap_write_low + 1024;
  ...
  RtlInitializeBitMap(&bitmap_write_low_header,
	  reinterpret_cast<PULONG>(bitmap_write_low), 1024 * 8);

  RtlSetBits(&bitmap_read_low_header, (ULONG)Msr::kIa32SysenterCs, 1);
  RtlSetBits(&bitmap_read_low_header, (ULONG)Msr::kIa32SysenterEip, 1);
  RtlSetBits(&bitmap_read_low_header, (ULONG)Msr::kIa32SysenterEsp, 1);
  RtlSetBits(&bitmap_read_low_header, (ULONG)Msr::kIa32FeatureControl, 1);

  RtlSetBits(&bitmap_write_low_header, (ULONG)Msr::kIa32SysenterCs, 1);
  RtlSetBits(&bitmap_write_low_header, (ULONG)Msr::kIa32SysenterEip, 1);
  RtlSetBits(&bitmap_write_low_header, (ULONG)Msr::kIa32SysenterEsp, 1);
  RtlSetBits(&bitmap_write_low_header, (ULONG)Msr::kIa32FeatureControl, 1);

  RTL_BITMAP bitmap_read_high_header = {};
  RtlInitializeBitMap(&bitmap_read_high_header,
	  reinterpret_cast<PULONG>(bitmap_read_high),
	  1024 * CHAR_BIT);
  RtlSetBits(&bitmap_read_high_header, ((ULONG)Msr::kIa32FsBase & 0xFFFF), 1);
  RtlSetBits(&bitmap_read_high_header, ((ULONG)Msr::kIa32GsBase & 0xFFFF), 1);
  RtlSetBits(&bitmap_read_high_header, ((ULONG)Msr::kIa32Lstar & 0xFFFF), 1);

  RTL_BITMAP bitmap_write_high_header = {};
  RtlInitializeBitMap(&bitmap_write_high_header,
	  reinterpret_cast<PULONG>(bitmap_write_high),
	  1024 * CHAR_BIT);
  RtlSetBits(&bitmap_write_high_header, ((ULONG)Msr::kIa32FsBase & 0xFFFF), 1);
  RtlSetBits(&bitmap_write_high_header, ((ULONG)Msr::kIa32GsBase & 0xFFFF), 1);
  RtlSetBits(&bitmap_write_high_header, ((ULONG)Msr::kIa32Lstar & 0xFFFF), 1);
...
_Use_decl_annotations_ static void VmpInitializeVm(
  ...
  processor_data->vmm_stack_limit =
      UtilAllocateContiguousMemory(KERNEL_LARGE_STACK_SIZE);
...
_Use_decl_annotations_ static bool VmpSetupVmcs(
  ...
  vm_procctl_requested.fields.cr3_load_exiting = false;
  ...
  error |= UtilVmWrite(VmcsField::kPageFaultErrorCodeMask, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kPageFaultErrorCodeMatch, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kTscOffset, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kTscOffsetHigh, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmExitMsrStoreCount, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmExitMsrStoreAddr, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmExitMsrLoadCount, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmExitMsrLoadAddr, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmEntryMsrLoadCount, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kCr3TargetCount, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kCr3TargetValue0, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kCr3TargetValue1, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kCr3TargetValue2, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kCr3TargetValue3, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmEntryIntrInfoField, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmEntryExceptionErrorCode, 0);//addbyme
  error |= UtilVmWrite(VmcsField::kVmEntryInstructionLen, 0);//addbyme
  ...
  error |= UtilVmWrite(VmcsField::kHostIa32Efer, UtilReadMsr(Msr::kIa32Efer));//addbyme
  ...
  if(VmpIsXsavesAvailable())
	UtilVmWrite64(VmcsField::kXssExitingBitmap, 0);
