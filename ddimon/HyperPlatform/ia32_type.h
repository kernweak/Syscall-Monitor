...
/// See: Enumeration of CPU Support For XSAVE Instruction And XSAVE Supported Features
union CpuFeaturesEaxXsave {
	ULONG32 all;
	struct {
		ULONG32 xsaveopt : 1;   //!< [0] XSAVEOPT instruction
		ULONG32 compact : 1;    //!< [1] Compaction extensions
		ULONG32 xgetbv : 1;     //!< [2] XGETBV with ECX=1
		ULONG32 xsaves : 1;     //!< [3] XSAVES/XRSTORS/IA32_XSS MSR
		ULONG32 reserved : 28;   //!< [31:4] reserved
	} fields;
};
static_assert(sizeof(CpuFeaturesEaxXsave) == 4, "Size check");
...
enum class VmcsField : unsigned __int32 {
  ...
  kPostedInterruptAddress = 0x00002016,
  kPostedInterruptAddressHigh = 0x00002017,
