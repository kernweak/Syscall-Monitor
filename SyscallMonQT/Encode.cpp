...
std::wstring UTF8ToUnicode(std::string &str)
{
	...
	result.resize(len);
    MultiByteToWideChar(CP_UTF8, 0, str.c_str(), (int)str.length(), (LPWSTR)result.data(), len);
	return result;
}
...
std::wstring ANSIToUnicode(std::string &str)
{
	...
	result.resize(len);
    MultiByteToWideChar(CP_ACP, 0, str.c_str(), (int)str.length(), (LPWSTR)result.data(), len);
	return result;
}
...
std::string UnicodeToUTF8(std::wstring &str)
{
	...
	result.resize(len);
    WideCharToMultiByte(CP_UTF8, 0, str.c_str(), (int)str.length(), (LPSTR)result.data(), len, NULL, NULL);
	return result;
}
...
std::string UnicodeToANSI(std::wstring &str)
{
	...
	result.resize(len);
    WideCharToMultiByte(CP_ACP, 0, str.c_str(), (int)str.length(), (LPSTR)result.data(), len, NULL, NULL);
	return result;
}
